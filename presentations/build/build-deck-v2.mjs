...
const skill = 'C:/Users/i.petrikin/.codex/plugins/cache/openai-primary-runtime/presentations/26.903.11726/skills/presentations';
...
function footer(s,n,note='Концепция интерфейса · демонстрационные данные',dark=false){
 ...
 text(s,String(n).padStart(2,'0')+' / 06',1465,854,90,28,17,dark?C.muted:C.gray);
}
...
const screens=[
 ...
 {n:4,slug:'completion',title:'Выполненные работы проходят проверку',blocks:[
  ['01','Фиксация факта','Исполнитель указывает операции, трудозатраты, материалы и результат работ.'],
  ['02','Проверка мастером','Мастер сверяет выполнение, подтверждает результат или возвращает на доработку.'],
  ['03','История техники','Сохраняются принятые работы, наработка, вложения и ответственные.']
 ],note:'Подтверждение — внутреннее действие ответственного сотрудника в системе. Не подразумеваются юридически значимая электронная подпись, актование в сторонней системе или финансовое закрытие. План 2,0 чел·ч и факт 2,2 чел·ч совпадают с суммой трех показанных операций. Материалы показаны как факт использования; складской процесс требует отдельного согласования.'},
...
for (const d of screens){
 ...
 await img(s,d.slug+'-branded.png',56,155,1020,680,'Концептуальный экран с айдентикой РЦ: '+d.title);
...
const finalPath=path.join(root,'output/presentations/Remontny-Centr-concept-6-slides-v2.pptx');
const result=await finalizePresentation({
 ...
 receiptPath:path.join(build,'renamed.validation.json')
});
